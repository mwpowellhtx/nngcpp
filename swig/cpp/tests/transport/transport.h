...
namespace nng {
    ...
    struct transport_fixture : public basic_fixture {
    ...
    private:
        ...
        std::string get_addr(int delta = 0);
        std::string get_next_addr(int delta = 1);
        std::string get_prev_addr(int delta = -1);
    };
}
...
//		Reset({ trantest_fini(&tt); });
//
//      ////TODO: TBD: there is not much point testing "scheme" or anything to do with nni_*, as this is under the hood so to speak, and we just want to drive the car...
//		//trantest_scheme(&tt);
//		trantest_conn_refused(&tt);
//		trantest_duplicate_listen(&tt);
//		trantest_listen_accept(&tt);
